...
# fix: capture original script args at the top level before any function calls,
# so check_root can forward them correctly on sudo re-exec
ORIGINAL_ARGS=("$@")
...
check_root() {
  if [ "$EUID" -ne 0 ]; then
    ...
    if command -v sudo &>/dev/null; then
      ...
      # fix: use ORIGINAL_ARGS instead of "$@" — "$@" here only contains the
      # args passed to check_root, not the original script args
...
comprehensive_cleanup() {
  ...
  # Uninstall drivers if Div-Linuwu-Sense folder exists
  if [ -d "Div-Linuwu-Sense" ]; then
    echo "Uninstalling drivers..."
    # fix: guard the cd so a failed directory change doesn't silently continue
    # in the wrong location for the rest of the cleanup
    cd Div-Linuwu-Sense || {
      echo -e "${RED}Error: Could not enter Div-Linuwu-Sense directory${NC}"
      systemctl daemon-reload
      return 1
    }
    if [ -f "Makefile" ]; then
...
install_drivers() {
  ...
  if [ ! -d "Div-Linuwu-Sense" ]; then
    echo -e "${RED}Error: Div-Linuwu-Sense directory not found!${NC}"
    echo "Please make sure the script is run from the same directory containing the Div-Linuwu-Sense folder."
    pause
    return 1
  fi

  # fix: guard the cd so a failure is caught immediately rather than running
  # make commands in the wrong directory
  cd Div-Linuwu-Sense || {
    echo -e "${RED}Error: Could not enter Div-Linuwu-Sense directory${NC}"
    pause
    return 1
  ...
  # fix: capture the result before cd so we always return to the parent
  # directory regardless of whether make succeeded or failed
  local make_result=$?
  cd .. || {
    echo -e "${RED}Error: Could not return to parent directory after driver build${NC}"
    pause
    return 1
  }
